...
Every adapter exposes the same transactional surface — `Transactional`, `Propagation`, `TransactionHost`, and the `runOnTransactionCommit`/`Rollback`/`Complete` lifecycle hooks — from a single import. The TypeORM adapter's module is a drop-in `TypeOrmModule` (use it instead of `@nestjs/typeorm`'s); the Prisma adapter's is `TransactionalModule`.
...
```ts
// app.module.ts — TypeOrmModule from THIS package, not @nestjs/typeorm
