...
- Transaction lifecycle hooks — `runOnTransactionCommit` / `runOnTransactionRollback` / `runOnTransactionComplete` (`typeorm-transactional` parity, built on CLS, no monkey-patching), re-exported by every adapter.
